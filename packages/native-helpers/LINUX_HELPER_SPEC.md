...
```
Supported event types: `keyDown`, `keyUp`, `flagsChanged`, `activeDisplayChanged`.

### Required RPC Methods

| Method | Params | Result | Description |
|---|---|---|---|
| `getAccessibilityContext` | `{ "editableOnly": boolean }` | `{ "context": AppContext \| null }` | Retrieves the current focused UI element and text selection. *(Wayland isolates clients; this may require compositor-specific DBus APIs or xdg-desktop-portal, else return null).* |
...
| `getAccessibilityTreeDetails` | *(varies)* | *(varies)* | Advanced accessibility tree inspection (best-effort on Linux). |
...
*To the autonomous agent: Follow these steps sequentially to build the `linux-helper`.*
